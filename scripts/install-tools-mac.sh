...
function install_cloudctl() {
	if [ ! -f $DESTINATION_DIR/cloudctl ]; then
		...
		RESPONSE=$(
			...
			https://"$CLUSTER_URL":8443/api/cli/cloudctl-darwin-amd64 -w "%{http_code}"
		)
...
function install_kubectl() {
	if [ ! -f $DESTINATION_DIR/kubectl ]; then
		...
		RESPONSE=$(
			...
			https://"$CLUSTER_URL":8443/api/cli/kubectl-darwin-amd64 -w "%{http_code}"
		)
		if [ "$RESPONSE" == 200 ]; then
			chmod +x kubectl
...
function install_istioctl() {
	if [ ! -f $DESTINATION_DIR/istioctl ]; then
		...
		RESPONSE=$(
			...
			https://"$CLUSTER_URL":8443/api/cli/istioctl-darwin-amd64 -w "%{http_code}"
		)
		if [ "$RESPONSE" == 200 ]; then
			chmod +x istioctl
...
function install_helm() {
	if [ ! -f $DESTINATION_DIR/helm ]; then
		...
		RESPONSE=$(
			curl --progress-bar -kLo helm-darwin-amd64.tar.gz \
			https://"$CLUSTER_URL":8443/api/cli/helm-darwin-amd64.tar.gz -w "%{http_code}"
		)
		if [ "$RESPONSE" == 200 ]; then
			tar -xzf helm-darwin-amd64.tar.gz
			mv darwin-amd64/helm "$DESTINATION_DIR"
			echo "$SUCCESS"
		else
			echo "$FAIL"
		fi
...
function install_calicoctl() {
	if [ ! -f $DESTINATION_DIR/calicoctl ]; then
		...
		RESPONSE=$(
			...
			https://"$CLUSTER_URL":8443/api/cli/calicoctl-darwin-amd64 -w "%{http_code}"
		)
		if [ "$RESPONSE" == 200 ]; then
			chmod +x calicoctl
...
# script should be called with address or hostname of cluster as argument
if [ "$#" -lt 1 ]; then
	echo
...
# script to print architecture on which it is running
